...
def test_episodic_memory_by_user(client, cheshire_cat):
    ...
    response = client.get(
        "/memory/recall/", params=params, headers={"user_id": "not_existing", "agent_id": cheshire_cat.id}
    )
    json = response.json()
    assert response.status_code == 200
    episodic_memories = json["vectors"]["collections"]["episodic"]
    ...
    response = client.get("/memory/recall/", params=params, headers={"user_id": "C", "agent_id": cheshire_cat.id})
    json = response.json()
    assert response.status_code == 200
    episodic_memories = json["vectors"]["collections"]["episodic"]
