...
def test_instantiation_discovery(march_hare_no_plugins: MarchHare):
    # Mad Hatter finds core_plugin
    assert list(march_hare_no_plugins.plugins.keys()) == ["core_plugin"]
    assert isinstance(march_hare_no_plugins.plugins["core_plugin"], Plugin)
    assert "core_plugin" in march_hare_no_plugins.load_active_plugins_from_db()

    # finds hooks
    assert len(march_hare_no_plugins.hooks.keys()) > 0
    for hook_name, hooks_list in march_hare_no_plugins.hooks.items():
    ...
    # finds tool
    assert len(march_hare_no_plugins.tools) == 1
    tool = march_hare_no_plugins.tools[0]
    ...
    active_plugins = march_hare_no_plugins.load_active_plugins_from_db()
...
def test_plugin_install(march_hare_no_plugins: MarchHare, plugin_is_flat):
    # install plugin
    new_plugin_zip_path = create_mock_plugin_zip(flat=plugin_is_flat)
    march_hare_no_plugins.install_plugin(new_plugin_zip_path)

    # archive extracted
    assert os.path.exists(os.path.join(utils.get_plugins_path(), "mock_plugin"))

    # plugins list updated
    assert list(march_hare_no_plugins.plugins.keys()) == ["core_plugin", "mock_plugin"]
    assert isinstance(march_hare_no_plugins.plugins["mock_plugin"], Plugin)
    assert (
        "mock_plugin" in march_hare_no_plugins.load_active_plugins_from_db()
    )  # plugin starts active

    # plugin is activated by default
    assert len(march_hare_no_plugins.plugins["mock_plugin"].hooks) == 3
    assert len(march_hare_no_plugins.plugins["mock_plugin"].tools) == 1

    # tool found
    new_tool = march_hare_no_plugins.plugins["mock_plugin"].tools[0]
    assert new_tool.plugin_id == "mock_plugin"
    assert id(new_tool) == id(march_hare_no_plugins.tools[1])  # cached and same object in memory!
    # tool examples found
    ...
    # hooks found
    new_hooks = march_hare_no_plugins.plugins["mock_plugin"].hooks
    hooks_ram_addresses = []
    for h in new_hooks:
    ...
    assert len(march_hare_no_plugins.hooks[mock_hook_name]) == 3  # one in core, two in mock plugin
    expected_priorities = [3, 2, 0]
    for hook_idx, cached_hook in enumerate(march_hare_no_plugins.hooks[mock_hook_name]):
...
def test_plugin_uninstall_non_existent(march_hare_no_plugins: MarchHare):
    # should not throw error
    assert len(march_hare_no_plugins.plugins) == 1  # core_plugin
    march_hare_no_plugins.uninstall_plugin("wrong_plugin")
    assert len(march_hare_no_plugins.plugins) == 1
...
@pytest.mark.parametrize("plugin_is_flat", [True, False])
def test_plugin_uninstall(march_hare_no_plugins: MarchHare, plugin_is_flat):
    # install plugin
    new_plugin_zip_path = create_mock_plugin_zip(flat=plugin_is_flat)
    march_hare_no_plugins.install_plugin(new_plugin_zip_path)

    # uninstall
    march_hare_no_plugins.uninstall_plugin("mock_plugin")

    # directory removed
    assert not os.path.exists(os.path.join(utils.get_plugins_path(), "mock_plugin"))

    # plugins list updated
    assert "mock_plugin" not in march_hare_no_plugins.plugins.keys()
    # plugin cache updated (only core_plugin stuff)
    assert len(march_hare_no_plugins.tools) == 1  # default tool
    for h_name, h_list in march_hare_no_plugins.hooks.items():
        assert len(h_list) == 1
